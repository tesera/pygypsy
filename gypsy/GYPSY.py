...
def densityNonSpatialAw(sp_Aw, SI_bh_Aw, bhage_Aw, N_Aw):
            ...
            if sp_Aw[0] == 'Aw' or \
               ...
               sp_Aw[0] == 'H':
                   ...
                   while abs(SDF_Aw0-SDF_Aw1)>0.00000001:
                       ...
                       b1=( ((SDF_Aw0/1000)**(-0.5))+ (numpy.sqrt(1+numpy.sqrt(50/(numpy.sqrt(SDF_Aw0)*numpy.log(50+1))))) )*numpy.log(50+1)
    ...
    return N_bh_Aw, SDF_Aw0

y_Aw=densityNonSpatialAw(sp_Aw, SI_bh_Aw, tage_Aw, N_Aw)
SDF_Aw = y_Aw[1]
N_bh_Aw=y_Aw[0]
print SDF_Aw , N_bh_Aw

 
      
def densityNonSpatialSb(sp_Sb, SI_bh_Sb, tage_Sb, N_Sb):
    if N_Sb>0:
        if bhage_Sb > 0 or SI_bh_Sb > 0:
            if sp_Sb[0] == 'Sb' or \
               sp_Sb[0] == 'Lt' or \
               sp_Sb[0] == 'La' or \
               sp_Sb[0] == 'Lw' or \
    ...
    return N_bh_Sb, SDF_Sb0
    
y_Sb=densityNonSpatialSb(sp_Sb, SI_bh_Sb, tage_Sb, N_Sb)
SDF_Sb = y_Sb[1]
N_bh_Sb=y_Sb[0]
print SDF_Sb , N_bh_Sb
     

              
def densityNonSpatialSw(sp_Sw, SI_bh_Sw, tage_Sw, SDF_Aw, N_Sw):
    if N_Sw>0:
        if bhage_Sw > 0 or SI_bh_Sw > 0:
            if sp_Sw[0] == 'Sw' or \
               sp_Sw[0] == 'Se' or \
               sp_Sw[0] == 'Fd' or \
               sp_Sw[0] == 'Fb' or \
    ...
    return N_bh_Sw, SDF_Sw0

y_Sw= densityNonSpatialSw(sp_Sw, SI_bh_Sw, tage_Sw, SDF_Aw, N_Sw)
SDF_Sw = y_Sw[1]
N_bh_Sw=y_Sw[0]
print SDF_Sw , N_bh_Sw
 

                
def densityNonSpatialPl(sp_Pl, SI_bh_Pl, tage_Pl, SDF_Aw, SDF_Sw, SDF_Sb, N_Pl):
    if N_Pl>0:
        if bhage_Pl > 0 or SI_bh_Pl > 0:               
            if sp_Pl[0] == 'P' or \
               sp_Pl[0] == 'Pl' or \
               sp_Pl[0] == 'Pj' or \
               sp_Pl[0] == 'Pa' or \
...
BA below is the Basal Area measured in the field and it should be data input into Gypsy in teh original listsp_Aw etc above

'''


total_N=(N_bh_Pl + N_bh_Aw + N_bh_Sw + N_bh_Sb)
SC_Aw = N_bh_Aw/total_N
SC_Sw = N_bh_Sw/total_N
SC_Sb = N_bh_Sb/total_N
SC_Pl = N_bh_Pl/total_N
...
def BasalAreaIncrementNonSpatialAw(sp_Aw, SC_Aw, SI_bh_Aw, N_bh_Aw, bhage_Aw):


    if N_bh_Aw>=0 and SI_bh_Aw>0:
        a1     =        0.751313
        a2     =        0.018847 
        a3     =        1.143762 
        a4     =        -0.03475 
        ...
        k1=(10**(-4))*a1*(bhage_Aw**2)*(numpy.exp(-a2*(bhage_Aw**(1/2+a1))))*(SC_Aw**a5)*((numpy.log(1+(N_bh_Aw*numpy.sqrt(1+bhage_Aw) )))**2)*SI_bh_Aw
    ...
    return BAinc_Aw
print BasalAreaIncrementNonSpatialAw(sp_Aw, SC_Aw, SI_bh_Aw, N_bh_Aw, bhage_Aw)
        
        
        
def BasalAreaIncrementNonSpatialSb(sp_Sb, SC_Sb, SI_bh_Sb, N_bh_Sb, bhage_Sb):


    if N_bh_Sb>=0 and SI_bh_Sb>0:
        a1      =       0.966285 
        a2      =       0.056315 
        a3      =        0.17191 
        k=(1+((N_bh_Sb**0.5)*(1+bhage_Sb)**0.5) )*(numpy.exp(-(N_bh_Sb/4)/10000))*(numpy.log(1+SI_bh_Sb)) /     ((1+bhage_Sb )**a2)
    ...
    return BAinc_Sb
print BasalAreaIncrementNonSpatialSb(sp_Sb, SC_Sb, SI_bh_Sb, N_bh_Sb, bhage_Sb)
           
    
    
def BasalAreaIncrementNonSpatialSw(sp_Sw, SC_Sw, SI_bh_Sw, N_bh_Sw, bhage_Sw):
    ...
    if N_bh_Sw>=0 and SI_bh_Sw>0:
        ...
        k2 = 1+ numpy.exp(1+ k + ( (numpy.log( 1+((N_bh_Sw**0.5)/10000) )) /2 )  + (a3* numpy.log(1+BA_Sw)) )
            
        m=(numpy.log (1+(N_bh_Sw*((1+bhage_Sw)**0.5) ))**2)* ((SI_bh_Sw)**0.5) * numpy.exp(-(N_bh_Sw/10)/10000) 
    ...
    return BAinc_Sw
print BasalAreaIncrementNonSpatialSw(sp_Sw, SC_Sw, SI_bh_Sw, N_bh_Sw, bhage_Sw)    
        
        
        
def BasalAreaIncrementNonSpatialPl(sp_Pl, SC_Pl, SI_bh_Pl, N_bh_Pl, bhage_Pl):
    ...
    if N_bh_Pl>=0 and SI_bh_Pl>0:
        ...
        k2= 1+numpy.exp( (k/2)+ numpy.log(1+((N_bh_Pl/3)/10000))  - (a3 * (SC_Pl**0.5)) + (a4*numpy.log(1+BA_Pl ) ))
         
        m1 = (1+a3+(SI_bh_Pl**a6) )* (N_bh_Pl**0.5) * numpy.exp(-(N_bh_Pl/3)/10000)
    ...
    return BAinc_Pl
print BasalAreaIncrementNonSpatialPl(sp_Pl, SC_Pl, SI_bh_Pl, N_bh_Pl, bhage_Pl)   
