...
def sp_prop_total(perc_sp):
    ...
    total_prop = {'PROP_Aw_data':0, 'PROP_Pb_data':0,
                  'PROP_Sw_data':0, 'PROP_Fb_data':0, 'PROP_Fd_data':0}
    for sp, perc in perc_sp:
        if sp == '' or sp == 0:
            break
        elif sp == 'Fd':
            total_prop['PROP_Fd_data'] = perc/100
        elif sp == 'Aw':
            total_prop['PROP_Aw_data'] = perc/100
        elif sp == 'Pb':
            total_prop['PROP_Pb_data'] = perc/100
        elif sp == 'Sw':
            total_prop['PROP_Sw_data'] = perc/100
        elif sp == 'Fb':
            total_prop['PROP_Fb_data'] = perc/100
    return total_prop
...
def merge_simulated_with_original_sp_prop(simulated_data, plot_data):
    ...
    selection = ['id_l1', 'PROP_Aw_data', 'PROP_Pb_data', 'PROP_Sw_data',
                 'PROP_Fb_data', 'PROP_Fd_data']
    plot_data_sliced = plot_data.loc[:, selection]
    plot_data_sliced['prop_Aw_class'] = plot_data_sliced.PROP_Aw_data + plot_data_sliced.PROP_Pb_data
    plot_data_sliced['prop_Sw_class'] = plot_data_sliced.PROP_Sw_data \
        + plot_data_sliced.PROP_Fb_data + plot_data_sliced.PROP_Fd_data
...
def disaggregate_simulation(simulated_data, plot_data):
    ...
    for gypsy_s, data_s in species:
        df['N_bh_%sT' % (data_s)] = df['N_bh_%sT' % gypsy_s] \
            * df['PROP_%s_data' % data_s] / df['prop_%s_class' %gypsy_s]
